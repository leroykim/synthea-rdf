    ...
    def convert(self, graph):
        ...
        with alive_bar(
            ...
            for index, row in self.__resource_df.iterrows():
                ...
                graph.add(
                    (dua, DUA.hasDataCustodian, organization_uri(row["dataCustodian"]))
                )
                ...
                graph.add(
                    (
                        ...
                        plain_literal(row["dataSecurityPlanAccess"]),
                    )
                )
                graph.add(
                    (
                        ...
                        plain_literal(row["dataSecurityPlanProtection"]),
                    )
                )
                graph.add(
                    (
                        ...
                        plain_literal(row["dataSecurityPlanStorage"]),
                    )
                )
                ...
                graph.add(
                    (term_and_termination, DUA.terms, plain_literal(row["terms"]))
                )
                graph.add(
                    (
                        term_and_termination,
                        DUA.terminationCause,
                        plain_literal(row["terminationCause"]),
                    )
                )
                graph.add(
                    (
                        ...
                        plain_literal(row["terminationEffect"]),
                    )
                )
                graph.add((dua, DUA.hasTermAndTermination, term_and_termination))

                # Permitted Use or Disclosure
                permitted_use_or_disclosure = permitted_use_or_disclosure_uri(
                ...
                graph.add(
                    (
                        ...
                        permitted_use_or_disclosure_uri(
                            row["permittedUseOrDisclosure"]
                        ),
                    )
                )
                ...
                for data in requested_data:
                    graph.add(
                        (dua, DUA.requestedData, plain_literal(synthea_uri(data)))
                    )
